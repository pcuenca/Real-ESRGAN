...
def degradation_simple_esrgan(img, sf=4, use_sharp=False, lq_patchsize=64, isp_model=None):
    """
    Simplified esrgan degradation.
    See https://github.com/xinntao/Real-ESRGAN/blob/master/realesrgan/data/realesrgan_dataset.py
    for real ESRGAN degradation strategy. For example, `sinc` is missing.
    Sharpening is applied to the hq image only.
    ----------
    img: HXWXC, [0, 1], its size should be large than (lq_patchsizexsf)x(lq_patchsizexsf)
    sf: scale factor
    use_shuffle: the degradation shuffle
